...
{
    /// <summary>Stage 정의와 진행 상태를 이용해 Stage 안의 Round 흐름을 제어한다.</summary>
    public class StageSimulator
    {
        private readonly CoreRoundSimulator _roundSimulator;

        /// <summary>시드 없는 Stage 시뮬레이터를 생성한다.</summary>
        public StageSimulator()
        {
            _roundSimulator = new CoreRoundSimulator();
        }

        /// <summary>고정 시드 기반 Stage 시뮬레이터를 생성한다.</summary>
        public StageSimulator(int seed)
        {
            _roundSimulator = new CoreRoundSimulator(seed);
        }
        ...
        public RoundState StartCurrentRound(StageProgressState stageProgressState)
        {
            ...
            return _roundSimulator.StartRound(currentRound.RuleContext);
        }
